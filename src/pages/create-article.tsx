...
import { LucideCalendarSearch, LucideTriangleAlert } from 'lucide-react';
import { format } from 'date-fns';
import { cn } from '../lib/utils';
import DOMPurify from 'dompurify';
import { Checkbox } from '../components/ui/checkbox';
import { Label } from '../components/ui/label';
import { createAnArticle } from '../request/article-request';
import { RadioGroup, RadioGroupItem } from '../components/ui/radio-group';
import { uploadImage } from '../request/media-request';
...
const CreateArticlePage = () => {
    ...
    const saveArticle = async (status: "draft" | "scheduled" | "published") => {
        if (editorInstance.current) {
            ...
            if (imageFile === null) {
                displayToast({
                    message: "Please select a fetaured image to proceed",
                    messageType: "error",
                    toast: toast
                })
                return
            ...
            const saveFeatureImageResponse = await uploadImage(formData)

            const axioResponse = saveFeatureImageResponse as AxiosResponse<UploadResponse, any>
    ...
    return (
        <>
            <div className="app-container w-full pb-16 relative">
                <div className="w-full flex justify-between text-gray-500 border-b border-gray-500 items-center">
                    ...
                    <div className="space-x-3 flex items-center">
                        <Button onClick={() => {saveArticle("draft")}} className="text-white hidden md:inline-flex">
                ...
                <div className="flex flex-col mt-14 md:flex-row gap-8">
                    <div className="">
                        ...
                        <div className="max-w-sm mt-14">
                            ...
                            <Input onChange={(e) => {setTitle(e.target.value)}} id="picture" placeholder="Enter article title" className="max-w-sm border border-gray-400 mt-2 py-6"/>
                        </div>
                    </div>
